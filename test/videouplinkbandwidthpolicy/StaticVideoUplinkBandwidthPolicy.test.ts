...
import StaticVideoUplinkBandwidthPolicy from '../../src/videouplinkbandwidthpolicy/StaticVideoUplinkBandwidthPolicy';

describe('StaticVideoUplinkBandwidthPolicy', () => {
  const expect: Chai.ExpectStatic = chai.expect;
  const assert: Chai.AssertStatic = chai.assert;
  const logger = new NoOpLogger(LogLevel.DEBUG);
  const selfAttendeeId = 'self-cb7cb43b';
  let policy: StaticVideoUplinkBandwidthPolicy;

  beforeEach(() => {
    policy = new StaticVideoUplinkBandwidthPolicy(selfAttendeeId);
  ...
  describe('updateConnectionMeric', () => {
    it('is no-opn for StaticVideoUplinkBandwidthPolicy', () => {
      policy.updateConnectionMetric({});
    });
  });

  describe('chooseMediaTrackConstraints', () => {
    it('returns empty MediaTrackConstraint for StaticVideoUplinkBandwidthPolicy', () => {
