...
export default class VideoAdaptivePolicy implements VideoDownlinkBandwidthPolicy {
  ...
  reset(): void {
    ...
    this.timeBeforeAllowSubscribeMs = VideoAdaptivePolicy.MIN_TIME_BETWEEN_SUBSCRIBE_MS;
    this.timeLastProbe = Date.now();
    this.timeBeforeAllowProbeMs = VideoAdaptivePolicy.MIN_TIME_BETWEEN_PROBE_MS;
    ...
    switch (newState) {
      ...
      case RateProbeState.ProbePending:
        if (
          ...
          now - this.timeLastProbe > VideoAdaptivePolicy.MIN_TIME_BETWEEN_PROBE_MS
        ) {
      ...
      case RateProbeState.Probing:
        if (now - this.timeProbePendingStart > this.timeBeforeAllowProbeMs) {
          ...
          this.timeBeforeAllowProbeMs = Math.min(
            ...
            VideoAdaptivePolicy.MAX_HOLD_BEFORE_PROBE_MS
          );
        } else {
  ...
  private handleProbe(
    ...
    if (this.downlinkStats.packetsLost > 0) {
      ...
      this.timeBeforeAllowSubscribeMs = VideoAdaptivePolicy.MIN_TIME_BETWEEN_SUBSCRIBE_MS * 3;
  ...
  private processBwPausedStreams(
    ...
    if (this.videoPreferences !== undefined && this.pauseTiles) {
      ...
      for (const preference of this.videoPreferences) {
        ...
        if (chosenStreams.findIndex(stream => stream.attendeeId === preference.attendeeId) === -1) {
          if (videoTile) {
